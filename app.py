...
def get_credentials():
    ...
    if not client_id:
        raise RuntimeError("ML_CLIENT_ID não está configurado no Render.")

    if not client_secret:
        raise RuntimeError("ML_CLIENT_SECRET não está configurado no Render.")
...
def save_tokens(data):
    ...
    if not access_token:
        raise RuntimeError("Mercado Livre não retornou Access Token.")
...
def refresh_access_token():
    with token_lock:
        ...
        if not refresh_token:
            raise RuntimeError(
                "Refresh Token não encontrado. Acesse /autorizar novamente."
            )
...
def get_access_token():
    ...
    if not access_token:
        raise RuntimeError(
            "Mercado Livre não está autorizado. Acesse /autorizar."
        )
...
def buscar_produtos():
    ...
    for consulta in consultas:
        try:
            ...
            print(
                f"[ML] Busca '{consulta}' -> HTTP {response.status_code}"
            )

            if not response.ok:
                print("[ML] ERRO:", response.text[:1000])
                continue
            ...
            print(f"[ML] EXCEÇÃO na busca '{consulta}': {repr(e)}")

    print(f"[ML] TOTAL DE PRODUTOS ENCONTRADOS: {len(produtos)}")
    return list(produtos.values())


def encontrar_promocao(item_id):
    produto = ml_get("/items/" + item_id)

    valor = produto.get("price")
    valor_original = produto.get("original_price")
    ...
    if valor < PRECO_MINIMO or valor > PRECO_MAXIMO:
        return None

    desconto = ((valor_original - valor) / valor_original) * 100
...
def enviar_telegram(produto):
    ...
    if not response.ok:
        raise RuntimeError("Erro no Telegram:\n" + response.text)
...
def home():
    ...
    <body>
        <div class="box">
            <h1>🔥 Mega Desconto</h1>
            <p>Bot de promoções do Mercado Livre</p>
            <hr>
            <a href="/autorizar">🔐 Conectar Mercado Livre</a>
            <a href="/ml-teste">🛒 Testar Mercado Livre</a>
            <a href="/buscar-teste">🔎 Testar busca</a>
            <a href="/oferta-teste">🔥 Buscar oferta</a>
            <a href="/telegram-teste">📲 Testar Telegram</a>
        </div>
    </body>
    </html>
    """
...
def callback():
    ...
    if not code:
        return "<h1>❌ Código não recebido</h1>", 400
    ...
    try:
        ...
        <p><a href="/ml-teste">🛒 Testar Mercado Livre</a></p>
        <p><a href="/buscar-teste">🔎 Testar busca</a></p>
        <p><a href="/oferta-teste">🔥 Buscar oferta</a></p>
        """
        ...
        <p><a href="/autorizar">🔐 Autorizar novamente</a></p>
        """, 500
...
def ml_teste():
    try:
        ...
        <p><b>Usuário:</b> {escape(str(nickname))}</p>
        <p><b>ID:</b> {escape(str(user_id))}</p>
        <hr>
        <a href="/buscar-teste">🔎 Testar busca</a>
        """
        ...
        <pre>{escape(str(e))}</pre>
        <a href="/autorizar">🔐 Autorizar novamente</a>
        """, 500
...
def buscar_teste():
    try:
        ...
        <p>Quantidade: <b>{len(produtos)}</b></p>
        <hr>
        """
        ...
        html += '<a href="/oferta-teste">🔥 Procurar oferta</a>'
        return html
...
def oferta_teste():
    try:
        ...
        for produto in produtos:
            ...
            try:
                promocao = encontrar_promocao(item_id)
                analisados += 1
            except Exception as e:
                print(f"[ML] Erro analisando {item_id}: {e}")
                continue
            ...
            <h2>{escape(str(titulo))}</h2>
            <p>De: R$ {promocao["preco_original"]:.2f}</p>
            <p>Por: R$ {promocao["preco"]:.2f}</p>
            <p>Desconto: {promocao["desconto"]:.1f}%</p>
            <p>📲 Oferta enviada para o Telegram.</p>
            """

        return f"""
        <h1>😕 Nenhuma promoção encontrada</h1>
        <p>Produtos analisados: <b>{analisados}</b></p>
        <p>Desconto mínimo: <b>{DESCONTO_MINIMO}%</b></p>
        <a href="/oferta-teste">🔄 Tentar novamente</a>
        """
...
def telegram_teste():
    try:
        ...
        <p>A mensagem foi enviada para o Telegram.</p>
        """
